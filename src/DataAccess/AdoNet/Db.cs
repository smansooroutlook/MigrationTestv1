...
namespace DataObjects.AdoNet
{
    ...
    public class Db
    {
        ...
        public Db(IOptions<ConfigCS> config) {
            connectionString = config.Value.DBServer;
        }

        public Db(string conn = null )
        {
            if (!string.IsNullOrEmpty(conn)) // index is 1 because 0 = localdb
                connectionString = conn;

            //var builder = new Configuration();
            //connectionString = _config.Value.DBServer;
            //connectionString = _config;
            //HostingEnvironment env = new HostingEnvironment();
            //Microsoft.Extensions.Configuration.ConfigurationBuilder config = ;
            //connectionString = new ConfigurationBuilder().AddJsonFile("appsettings.json").Build()["ConfigCS:DBServer"];
        }
